...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/06/20 15:57:22 by festermo          #+#    #+#             */
/*   Updated: 2020/07/02 18:27:59 by student          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_str(char *str, char *flags, int *star, int *ret_val)
{
	...
	if (ft_strchr(flags, '.'))
	{
		...
		{
			if (width != 0)
				*ret_val += ft_str_wnz(&width, str, &length);
			else
				*ret_val += ft_str_wnz2(&length);
		}
		else
			*ret_val += ft_str_wzall(width, str, length);
	}
	else
		*ret_val += ft_nw_str(&length, str, flags);
	if (flag == 1)
		free(str);
}
